...
# %% split
'''
split was used to split raw files and perform color correction on the complete dataset.
This is a deprecated function since color correcting only the localizations is much faster.
Remains in the code since it can be handy for visualization purposes or when not working with localizations.
'''
...
def _split(args):
    ...
    from glob import glob
    from tqdm import tqdm
    from spit import tools as tools
    ...
    # print all parameters
    ws = '  '
    print('Split/Alignment - Parameter Settings:')
    print(f'{"No":<6} | {ws+"Label":<18} | {ws+"Value":<10}')
    for index, element in enumerate(vars(args)):
        print(f'{index+1:<6} | {ws+element:<18} | {ws+str(getattr(args,element)):<10}')
    print('--------------------------------------------------------')
    ...
    for path in tqdm(paths, desc='Splitting files...'):
        movieSplit, infoSplit = tools.split_movie(
            path, transform=args.transform)
        dtype = np.dtype(infoSplit[0]["Data Type"])

        for j in range(3):
            head, tail = os.path.split(path)
            root, ext = os.path.splitext(tail)
            if args.transform:
                newname = root + '_ch'+str(j)+'_corr' + ext
            else:
                newname = root + '_ch'+str(j) + ext
            newpath = head + '//' + newname

            if ext == '.raw':
                movieSplit[j].astype(dtype).tofile(newpath)

            if ext == '.tif':
                if movieSplit[0].shape[0] > 1:
                    imlist = []
                    for m in movieSplit[j]:
                        imlist.append(Image.fromarray(m))
                    imlist[0].save(newpath, save_all=True,
                                   append_images=imlist[1:])
                else:
                    Image.fromarray(movieSplit[j][0]).save(newpath)

            tools.dump_info(newpath, infoSplit[0])

        print(f'Split file {path}.')
        if args.transform == True:
            print(f'Corrected file {path}.')


# %% simulate
def _sim(args):
    import os
    from spit import simulate as sim
    # print params
    ws = '  '
    ...
    if args.path is None:
        ...
        pathCandidate = args.path

    # for each Simulation run, a new folder is created for all the files
    j = 0
    path = os.path.join(pathCandidate, f'Sim{j:05}')
    ...
    length = args.length
    k1 = args.onrate
    k1_t = args.onrate_trimer
    k2 = args.offrate
    k2_t = args.offrate_trimer
    channels = args.channels
    dist = args.distance
    dt = args.dt
    hetero = args.hetero
    kbleach = args.kbleach
    Dmonomer = args.Dmonomer
    Ddimer = args.Ddimer
    Dtrimer = args.Dtrimer
    locsd = args.locsd

    if args.type == 'D':
        (dataframeSim, info, dt) = sim.simulate_dimers(path,
                                                       density,
                                                       frames,
                                                       length,
                                                       k1,
        ...
        (dataframeSim, info, dt) = sim.simulate(path,
                                                density,
                                                frames,
                                                length,
                                                k1,
    ...
    oligomerCounter = sim.count_oligomers(dataframeSim, dt)
    sim.plot_oligomer_counter(path, oligomerCounter, dt)
    sim.plot_interaction_durations(path, dataframeSim, dt)
    sim.plot_track_length(path, dataframeSim, dt)

    # ############################################################# Merging channels
    # dfSimSwift0 = pd.read_csv(os.path.join(dirpath+'sim_locs0.GTtracked.csv'))
    # dfSimSwift1 = pd.read_csv(os.path.join(dirpath+'sim_locs1.GTtracked.csv'))

    # dfSimAllSwift = sim.mergeSimTracks(dfSimSwift0,dfSimSwift1)
    # dfSimAllSwift.to_csv(os.path.join(dirpath+'sim_locsAll.GTtracked.csv'), index=False)
# %% main
...
def main():
    ...
    split_parser = subparsers.add_parser(
        'split', help='split the channels and correct alignment'
    )
    ...
    sim_parser.add_argument(
        '-t', '--type', type=str, default='B',
        help='D to simulate dimers, T to simulate trimers or B for both'
    )
    sim_parser.add_argument(
        '-den', '--density', type=float, default=0.8,
        help='density of particles per channel per um2'
    )
    sim_parser.add_argument(
    ...
    sim_parser.add_argument(
        '-l', '--length', type=float, default=10,
        help='length of the simulated box in um'
    )
    sim_parser.add_argument(
        '-k1', '--onrate', type=float, default=20,
        help='interaction rate for particles that are closer than dist per s'
    )
    sim_parser.add_argument(
        '-k1_t', '--onrate_trimer', type=float, default=80,
        help='effective on-rate of 3 colocalizing particles'
    )
    sim_parser.add_argument(
        '-k2', '--offrate', type=float, default=0.02,
        help='off-rate of two interacting particles in 1/seconds'
    )
    sim_parser.add_argument(
        '-k2_t', '--offrate_trimer', type=float, default=0.005,
        help='off-rate of 3 interacting particles'
    )
    sim_parser.add_argument(
        '-ch', '--channels', type=int, default=2
    )
    sim_parser.add_argument(
        '-dist', '--distance', type=float, default=0.05,
        help='distance threshold where interaction can happen in um'
    )
    sim_parser.add_argument(
        '-dt', '--dt', type=float, default=0.01,
        help='timestep in seconds'
    )
    sim_parser.add_argument(
        '-het', '--hetero', action='store_false',
        help='homo- or hetero-interaction'
    )
    sim_parser.add_argument(
        '-kbl', '--kbleach', type=float, default=0,
        help='Bleaching rate in /s'
    )
    sim_parser.add_argument(
        '-Dm', '--Dmonomer', type=float, default=1,
        help='Diffusion constant of monomers in um2/s'
    )
    sim_parser.add_argument(
        '-Dd', '--Ddimer', type=float, default=0.7,
        help='Diffusion constant of dimers in um2/s'
    )
    sim_parser.add_argument(
        '-Dt', '--Dtrimer', type=float, default=0.7,
        help='Diffusion constant of trimers'
    )
    sim_parser.add_argument(
        '-locsd', '--locsd', type=float, default=0.035,
        help='Localization precision standard error in um'
    )

    # # visualize
    # visualize_parser = subparsers.add_parser(
    #     'visualize', help='visualize trajectories'
    #     )
    # visualize_parser.add_argument(
    #     'files', nargs='?',
    #     help='directory with linked_all.csv, colocEvents.csv, pairIDs.csv to visualize'
    #     )
    # visualize_parser.add_argument(
    #     '-all', '--plot_all', action='store_true',
    #     help='plot all trajectories'
    #     )
    # visualize_parser.add_argument(
    #     '-plt', '--plot', action='store_true',
    #     help='plot two trajectories'
    #     )
    # visualize_parser.add_argument(
    #     '-int', '--interact', action = 'store_true',
    #     help = 'plot only interacting particles'
    #     )
    # visualize_parser.add_argument(
    #     '-ani', '--animate', action='store_true',
    #     help='animate two trajectories'
    #     )
    # visualize_parser.add_argument(
    #     '-lb1', '--label1', type=str, default='particle 1'
    #     )
    # visualize_parser.add_argument(
    #     '-lb2', '--label2', type=str, default='particle 2'
    #     )
    # visualize_parser.add_argument(
    #     '-fstart', '--frame_start', type=int, default=False
    #     )
    # visualize_parser.add_argument(
    #     '-fend', '--frame_end', type=int, default=False
    #     )
    # visualize_parser.add_argument(
    #     '-intr', '--interval', type=int, default=100
    #     )
    # visualize_parser.add_argument(
    #     '-dpi', '--dpi', type=int, default=150
    #     )
    # visualize_parser.add_argument(
    #     '-col1', '--color_ch1', default=None
    #     )
    # visualize_parser.add_argument(
    #     '-col2', '--color_ch2', default=None
    #     )
    # visualize_parser.add_argument(
    #     '-o', '--animate_overlay', action='store_true',
    #     help='animate two trajectories overlayed on raw image'
    #     )
    # visualize_parser.add_argument(
    #     '-s', '--sim', action = 'store_true', help = 'simulated data used ?'
    #     )

    # # stats
    # stats_parser = subparsers.add_parser(
    #     'stats', help='overview stats'
    #     )
    # stats_parser.add_argument(
    #     'files', nargs='?'
    #     )
    # stats_parser.add_argument(
    #     '-fps', '--fps', type=int, default=20
    #     )
    # stats_parser.add_argument(
    #     '-px', '--px_size', type=float, default=0.108
    #     )
    # stats_parser.add_argument(
    #     '-ht', '--height', type=int, default=675
    #     )
    # stats_parser.add_argument(
    #     '-w', '--width', type=int, default=666
    #     )
    # stats_parser.add_argument(
    #     '-lag', '--max_lagtime', type=int, default=5
    #     )
    # stats_parser.add_argument(
    #     '-s', '--saved', action = 'store_true',
    #     help = 'to load an existing file with the stats'
    #     )
