...
def main(): 
    directory_path = r'D:\Data\20250408_GCL009\selected2'
...
def main(): 
    ...
    for path in directory_names:
        if os.path.isdir(path) and not os.path.isdir(os.path.join(directory_path, 'output', path.replace(directory_path, '')[1:])):
            # print(os.path.join(directory_path, 'output', path.replace(directory_path, '')[1:]))
...
def preprocess(fol, directory_path):            
    ...
    save_folder = os.path.join(directory_path, 'output', fol.replace(directory_path, '')[1:])
    ...
    if result_txt['Mode'] == 'Sequence':
        pattern = tools.get_pattern(result_txt)
        for pat, ch in pattern.items():
            ...
            for i in ch:
                ...
                if i.strip() in ['405nm', '488nm']:
                    im = np.array(list(map(lambda img: tools.h_affine_transform(img, Hr), image)))
                elif i.strip() in ['638nm']:
                    im = np.array(list(map(lambda img: tools.h_affine_transform(img, Hl), image)))
                else:
                    im = np.copy(image)
                cropped_im  = im[:, ylim[0]:ylim[1], xlim[0]:xlim[1]].astype(np.uint16)
                with open(os.path.join(save_folder, pat+"_"+i.strip()+'.yaml'), "w") as file:
                    _yaml.dump_all(inf, file, default_flow_style=False)
                save = os.path.join(save_folder, pat+"_"+i.strip()+'.tif')
                imageio.mimwrite(save, cropped_im)
                del im, cropped_im  # Free memory
            del d  # Free memory
    elif result_txt['Mode'] == 'VCR':
        pattern = tools.get_VCR_pattern(result_txt)
        ...
        for ch, presence in pattern.items():
            if presence:
                ...
                if ch in ['405nm', '488nm']:
                    ...
                    im = np.copy(image)
                cropped_im  = im[:, ylim[0]:ylim[1], xlim[0]:xlim[1]].astype(np.uint16)
                with open(os.path.join(save_folder, ch+'.yaml'), "w") as file:
                    _yaml.dump_all(inf, file, default_flow_style=False)
                save = os.path.join(save_folder, ch+'.tif')
                imageio.mimwrite(save, cropped_im)
                del im, cropped_im  # Free memory
            del d  # Free memory
    print('Finished with files in', fol.replace(directory_path, '')[1:])
